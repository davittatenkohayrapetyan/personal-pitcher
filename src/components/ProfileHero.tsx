...
export default function ProfileHero() {
  return (
    <section
      ...
      className="rounded-3xl border border-slate-400/15 bg-slate-900/80 p-6 shadow-xl shadow-black/25 sm:p-7 lg:p-8"
    >
      {/* Avatar row */}
      <div className="flex items-start gap-5 sm:gap-6">
        {/* Profile photo */}
        <div className="relative h-24 w-24 flex-shrink-0 overflow-hidden rounded-2xl ring-2 ring-violet-400/25 shadow-lg lg:h-28 lg:w-28">
          <Image
      ...
      {/* Skill badges */}
      <ul className="mt-5 flex flex-wrap gap-2" aria-label="Key areas of expertise">
        {BADGES.map((b) => (
          <li
            key={b.label}
            className="inline-flex items-center gap-1.5 rounded-full border border-slate-400/15 bg-slate-800/60 px-3 py-1.5 text-xs font-medium text-slate-200"
          >
      ...
      {/* Social links */}
      <div className="mt-5 flex flex-wrap items-center gap-3 border-t border-slate-400/10 pt-5">
        <a
          href="https://github.com/davittatenkohayrapetyan"
          target="_blank"
          rel="noopener noreferrer"
          className="inline-flex items-center gap-2 rounded-xl border border-slate-400/15 bg-slate-800/50 px-4 py-2 text-sm font-medium text-slate-200 transition-colors hover:border-violet-400/40 hover:text-white"
        ...
        <a
          ...
          aria-label="Davit Hayrapetyan personal website (opens in new tab)"
        >
