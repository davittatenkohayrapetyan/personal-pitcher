 ...
 * text that is sent.
 *
 * The wording is Davit's, and it does three things in one sentence, in this
 * order on purpose. **"generated by DAVO"** says plainly that an automated
 * pipeline found the role and wrote the first version — stated outright rather
 * than left to be discovered, because being caught at it later is the outcome
 * that costs a reply. **"reviewed and sent by me"** is where the responsibility
 * lands, and it is the clause the rest of this codebase exists to make true.
 * **The link** turns the disclosure into an invitation: for the roles this
 * system looks for, the machine that sent the letter is itself the work sample.
 */
export function disclosureLine(): string {
  return (
    env('OUTREACH_DISCLOSURE') ??
    'This email was generated by DAVO, my personal pitcher at https://davithayrapetyan.dev, ' +
    'and reviewed and sent by me — there is more about my work, and about how DAVO works, on the site.'
  );
}
